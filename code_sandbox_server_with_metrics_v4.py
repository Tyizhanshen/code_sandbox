...
_SEMAPHORE_LIMIT = int(os.environ.get(
    "SANDBOX_SEMAPHORE_LIMIT", str(max(_BASE_CPU * 8 // _NUM_WORKERS, 50))
...
_MONITORED_PATHS = frozenset({
    "/model_code", "/custom_code",
    "/custom_code_A", "/custom_code_B", "/custom_code_C",
    "/custom_code_D", "/custom_code_E",
})


class DisconnectAwareMiddleware:
    """
    Pure ASGI middleware (outermost layer). Two responsibilities:
    1. Processing gauge: replaces the old @app.middleware("http") BaseHTTPMiddleware
    2. Disconnect detection: wraps `receive` to passively capture http.disconnect
       events via receive_wrapper. Exposes scope["_disconnect_event"] for route
       handlers to poll.

    NOTE: We do NOT actively read from raw `receive()` in a background task,
    because BaseHTTPMiddleware (PrometheusMiddleware) may also have an internal
    reader on `receive()`, causing a race condition. Instead, we passively
    intercept messages flowing through receive_wrapper.
    """
    ...
    async def __call__(self, scope, receive, send):
        ...
        async def receive_wrapper():
            """Intercept all ASGI receive messages. If any caller
            (BaseHTTPMiddleware, Starlette internals, etc.) reads
            http.disconnect, we capture it here."""
        ...
        # --- Processing gauge ---
        path = scope.get("path", "")
        monitored = path in _MONITORED_PATHS
        if monitored:
...
# IMPORTANT: Starlette builds middleware via reversed() iteration,
# so add_middleware() called FIRST → outermost in the stack.
# DisconnectAwareMiddleware MUST wrap outermost to capture raw ASGI receive
# BEFORE PrometheusMiddleware (BaseHTTPMiddleware) replaces the receive channel.
app.add_middleware(DisconnectAwareMiddleware)  # outermost (registered first)
app.add_middleware(
    PrometheusMiddleware,
    labels=custom_labels,
    group_paths=True,
    skip_paths=["/metrics"],
)  # inner (registered second)
    ...
    try:
        ...
        if parent:
            try:
                ...
                pass
        # IMPORTANT: Do NOT call psutil.wait_procs() here!
        # It internally calls os.waitpid() which reaps the zombie process.
        # This steals the zombie from asyncio's ChildWatcher (which also uses
        # os.waitpid via SIGCHLD), causing proc.communicate() to hang forever.
        # Let asyncio handle zombie reaping through its own event loop.
        return
    except ImportError:
...
async def _run_code(request: Request, req: RunReq):
    ...
    try:
        ...
        queue_timeout = (req.run_timeout or 30) + 30
        ...
        try:
            ...
            try:
                while not sem_task.done():
                    ...
                    # Use _disconnect_event (set by DisconnectAwareMiddleware)
                    # instead of request.is_disconnected() which is broken
                    # by BaseHTTPMiddleware (PrometheusMiddleware).
                    _disc_event = request.scope.get("_disconnect_event")
                    _client_gone = False
                    if _disc_event:
                        _client_gone = _disc_event.is_set()
                    else:
                        try:
                            _client_gone = await request.is_disconnected()
                        except Exception:
                            pass
        ...
        try:
            ...
            try:
                ...
                async def _check_disconnect():
                    """Hybrid disconnect detection: polls both the middleware
                    event AND request.is_disconnected() every 2s.

                    The middleware event catches disconnects that flow through
                    receive_wrapper. is_disconnected() catches disconnects that
                    Starlette/BaseHTTPMiddleware detects internally.
                    Either mechanism triggering is sufficient."""
                    nonlocal _disconnect_detected
                    _disc_event = request.scope.get("_disconnect_event")
                    while True:
                        await asyncio.sleep(2)
                        disconnected = False
                        # Method 1: middleware event
                        if _disc_event and _disc_event.is_set():
                            disconnected = True
                        # Method 2: Starlette is_disconnected
                        if not disconnected:
                            try:
                        ...
                        if disconnected:
                            ...
                            logger.info(
                                "Client disconnect detected during execution, killing pid=%s (event=%s)",
                                proc.pid, _disc_event.is_set() if _disc_event else "N/A"
                            )
...
@app.get("/debug/test_disconnect")
async def debug_test_disconnect(request: Request):
    """Diagnostic endpoint: hold connection open, poll disconnect detection.
    Client should disconnect mid-way to verify detection works.
    
    Usage: curl --max-time 5 http://host:port/debug/test_disconnect
    Then check the returned JSON (or server logs if client disconnected).
    """
    disc_event = request.scope.get("_disconnect_event")
    results = []
    for i in range(30):
        await asyncio.sleep(1)
        event_set = disc_event.is_set() if disc_event else None
        try:
            is_disc = await request.is_disconnected()
        except Exception as e:
            is_disc = f"error: {e}"
        results.append({"t": i + 1, "event_set": event_set, "is_disconnected": is_disc})
        if event_set:
            logger.info("debug_test_disconnect: disconnect detected at t=%ds", i + 1)
            break
    return {
        "middleware_registered": disc_event is not None,
        "polls": results,
    }


@app.post("/debug/test_disconnect")
async def debug_test_disconnect_post(request: Request):
    """Same as GET but for POST — tests disconnect detection with body parsing.

    Usage: curl --max-time 5 -X POST -H 'Content-Type: application/json' \
           -d '{"test": true}' http://host:port/debug/test_disconnect
    """
    # Read body to simulate real endpoints
    ...
    for i in range(30):
        ...
        try:
            ...
            is_disc = f"error: {e}"
        results.append({"t": i + 1, "event_set": event_set, "is_disconnected": is_disc})
        if event_set or is_disc is True:
            logger.info("debug_test_disconnect(POST): detected at t=%ds (event=%s, is_disc=%s)", i + 1, event_set, is_disc)
            break
    return {
        "method": "POST",
        "body_size": len(body),
